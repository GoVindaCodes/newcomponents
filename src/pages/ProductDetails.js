...
import { showingTranslateValue } from "utils/translate";
import SettingServices from "services/SettingServices";
import { Badge, Button, Pagination, Table, TableCell, TableContainer, TableFooter, TableHeader } from "@windmill/react-ui";
...
const ProductDetails = () => {
  ...
  const handleReviewSubmit = (formData) => {
    // Implement functionality to submit review data to backend
  ...
  return (
    <>
      ...
      <div className="p-5 md:p-8">
        {/* <h2 className="text-lg font-semibold mb-3">{t("LeaveReview")}</h2> */}
      ...
      <div className="p-5 md:p-8">
        {/* <button onClick={handleTogglePaymentModal}>Open Payment Methods</button> */}
        <button onClick={handleTogglePaymentModal}>{t("Open Payment Methods")}</button>
      </div>
      <div isOpen={isPaymentModalOpen} onClose={handleTogglePaymentModal}>
        <PaymentMethods
      ...
      <div>
        ...
        <Modal open={isOrderModalOpen} onClose={handleCloseOrderModal}>
          <OrderProcessModal onSubmitOrder={handleSubmitOrder} />
        </Modal>
        {isOrderProcessVisible && <OrderProcessModal onClose={() => setIsOrderProcessVisible(false)} />}
        {isOrderProcessVisible && <OrderProcessModal onClose={() => setIsOrderProcessVisible(false)} />}
      </div>
      {reviews.length > 0 && <FeedbackMonitor productId={id} reviews={reviews} />}
      {data?.isCombination && variantTitle?.length > 0 && !loading && (
        <>
          <PageTitle>{t("ProductVariantList")}</PageTitle>
          <TableContainer className="mb-8 rounded-b-lg">
